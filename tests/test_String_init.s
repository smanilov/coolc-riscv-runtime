...
Main.main:
    ...
    la t0, _funky_string
    bne a0, t0, Main.main.not_ok  # the string is the same

    lw t0, 0(a0)
    li t1, 4
    bne t0, t1, Main.main.not_ok  # the tag has not changed

    lw t0, 4(a0)
    li t1, 5
    bne t0, t1, Main.main.not_ok  # the size has not changed

    # dispatch_table is not set by _init, as it is assumed to be always correct
    # (set by Object.copy, when the String object was allocated)
    lw t0, 8(a0)
    li t1, 42
    bne t0, t1, Main.main.not_ok

    lw t0, 12(a0)
    beqz t0, Main.main.not_ok  # the length has changed

    lw t0, 12(t0)
    bnez t0, Main.main.not_ok  # the length is zero
...
_funky_string:
    .word 4  # class tag;       4 for String
    .word 5  # object size;     6 words (16 + 8 bytes); GC tag not included
    .word 42 # crazy dispatch table; important for test
    .word 0 # first attribute; pointer length; null so that String.init can set it
...
_ok_string:
    ...
    .word String_dispTab
    .word _ok_string.length # first attribute; pointer length
...
_not_ok_string:
    ...
    .word String_dispTab
    .word _not_ok_string.length # first attribute; pointer length
    .string "bad\n" # includes terminating null char
    .byte 0
    .byte 0
    .byte 0
